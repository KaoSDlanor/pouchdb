...
var flatten$1 = function (...args) {
  ...
  for (var i = 0, len = args.length; i < len; i++) {
    ...
    if (Array.isArray(subArr)) {
      res$$1 = res$$1.concat(flatten$1.apply(null, subArr));
    } else {
      res$$1.push(subArr);
    }
  }
...
function mergeObjects(arr) {
  var res$$1 = {};
  for (var i = 0, len = arr.length; i < len; i++) {
    res$$1 = $inject_Object_assign(res$$1, arr[i]);
  }
  return res$$1;
}
...
function pick$1(obj$$1, arr) {
  ...
  for (var i = 0, len = arr.length; i < len; i++) {
    ...
    var value = getFieldFromDoc(obj$$1, parsedField);
    if (typeof value !== 'undefined') {
      setFieldInDoc(res$$1, parsedField, value);
    }
  }
  return res$$1;
}
...
function getKeyFromDoc(doc, index) {
  ...
  for (var i = 0; i < index.def.fields.length; i++) {
    ...
    res$$1.push(getFieldFromDoc(doc, parseField(field)));
  }
  return res$$1;
}
...
function getMultiFieldQueryOpts(selector, index) {
  ...
  if (typeof inclusiveStart !== 'undefined') {
    res$$1.inclusive_start = inclusiveStart;
  }
  if (typeof inclusiveEnd !== 'undefined') {
    res$$1.inclusive_end = inclusiveEnd;
  }
...
function doAllDocs(db, originalOpts) {
  ...
  return db.allDocs(opts)
    .then(function (res$$1) {
      ...
      res$$1.rows = res$$1.rows.filter(function (row) {
      ...
      res$$1.rows = res$$1.rows.slice(0, opts.limit);
      return res$$1;
    });
}
  ...
  return getIndexes$1(db).then(function (getIndexesRes) {
    ...
    return Promise.resolve().then(function () {
      ...
      if (opts.inclusive_start === false) {
        ...
        res$$1.rows = filterInclusiveStart(res$$1.rows, opts.startkey, indexToUse);
      }

      if (queryPlan.inMemoryFields.length) {
        // need to filter some stuff in-memory
        res$$1.rows = filterInMemoryFields(res$$1.rows, requestDef, queryPlan.inMemoryFields);
      }

      var resp = {
        docs: res$$1.rows.map(function (row) {
          var doc = row.doc;
          if (requestDef.fields) {
